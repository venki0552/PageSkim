...
CLI that checks a [PageSkim](https://github.com/pageskim/pageskim) sibling against its HTML page: structural conformance, spec version, content-hash staleness, chunk-anchor resolution, header budget, facts wellformedness, and **sentence-level grounding** — every sentence in the sibling must be derivable from the human-visible HTML, which catches cloaked content and injected prompt-instructions.
