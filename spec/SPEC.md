...
series-prev: {url}                                    (opt)
series-next: {url}                                    (opt)
hash: sha256:{16 lowercase hex — truncated digest, §10}
...
header (§4). TOC descriptions are at most the first 6 words of the chunk
summary and at most 48 characters. Generators MUST apply this deterministic
overflow sequence until the header fits the cap:

1. Drop TOC descriptions (`- {id}` lines).
2. Switch to grouped TOC (§15, huge pages).

A header that still exceeds 150 units after step 2 is a generator bug;
validators flag headers over the cap as errors and over the target as
warnings. (The SHA-256 hash is truncated to 16 hex characters — §10 — 
precisely so identity metadata leaves budget for the TOC.)
...
3. `hash = "sha256:" + first 16 characters of lowercase-hex(SHA-256(UTF-8(T)))`.

The truncation to 16 hex characters (64 bits) is deliberate: the hash exists
to detect *staleness*, not to resist adversaries — a full digest would spend
~25 tokens of header budget on entropy nobody needs. A validator recomputes
the digest from the HTML and compares prefixes; a mismatch means the sibling
...
The generated combined sibling (`page.llm.md`) is committed verbatim as the
golden fixture
[`packages/core/test/fixtures/expected/article-infobox.llm.md`](../packages/core/test/fixtures/expected/article-infobox.llm.md)
(regenerated by `npm run fixtures:update -w @pageskim/core`). Its header,
facts, and first chunk:
...
Hand-validation walkthrough: this page's TOC dropped its descriptions — with
nine chunks, the header exceeded the 150-unit cap in `full` mode, so the §9
overflow sequence stepped to `ids` (the header lands at 124 budget units /
~137 o200k tokens). Every chunk ID equals the `id` attribute of its `h2` in
the HTML; every infobox row appears as a facts key-value; no text from the
site header, fundraising banner, contents box, asides, or footer appears
anywhere; the summary is the page's `meta description` verbatim; `updated`
comes from `article:modified_time`; the hatnote ("Pharos" redirects here…)
is retained as page content by design.
